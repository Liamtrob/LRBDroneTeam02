...
class HighFlyers():
    ...
    def fly_home(self, drone):
        if self.x_distance > 0: #need to move backward
            self.drone.move_back(self.x_distance)
        if self.x_distance < 0:
            self.drone.move_forward(self.x_distance)     
        if self.y_distance > 0: #need to move left
            self.drone.move_left(self.y_distance)
        if self.y_distance < 0:
            self.drone.move_right(self.y_distance)
